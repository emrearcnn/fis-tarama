...
vergi_dairesi_patterns = [
    re.compile(r"(?:Vergi\s*Dairesi|VD|V\.D\.|DAIRE)[:\s]*([A-ZÇĞIİÖŞÜ][a-zçğıiöşü\s]+)", re.IGNORECASE),
    re.compile(r"([A-ZÇĞIİÖŞÜ][a-zçğıiöşü\s]+)\s*(?:Vergi\s*Dairesi|VD|V\.D\.)", re.IGNORECASE),
    re.compile(r"DAIRE[:\s]*([A-ZÇĞIİÖŞÜ][A-ZÇĞIİÖŞÜa-zçğıiöşü\s]+)", re.IGNORECASE),
    re.compile(r"([A-ZÇĞIİÖŞÜ][A-ZÇĞIİÖŞÜa-zçğıiöşü\s]+)\s*DAİRESİ", re.IGNORECASE)
]
...
def process_receipt():
    try:
        ...
        def find_vergi_dairesi(text):
            for pattern in vergi_dairesi_patterns:
                match = pattern.search(text)
                if match:
        ...
        vergi_dairesi = find_vergi_dairesi(text)
...
def export_excel():
    try:
        ...
        # Sütun sıralaması
        columns = ['ID', 'İşlem Tarihi', 'Dosya Adı', 'VKN/TCKN', 'Vergi Dairesi', 'Tarih', 'Toplam Tutar', 'KDV Tutarı']
        df = df[columns]
